...
export async function GET() {
  try {
    ...
    const querySnapshot = await getDocs(q).catch((err) => {
      // Create friendly error with required trace details
      console.error('Firestore Read Failure:', err);
      throw err;
    });
    
    let txs = querySnapshot.docs.map(doc => {
      const data = doc.data();
      return {
        id: doc.id,
        ...data,
    ...
    if (txs.length === 0) {
      ...
      const initialTransactions = [
        {
          ...
          timestamp: Timestamp.fromDate(new Date(Date.now() - 5 * 60 * 1000)) // 5 mins ago
        },
        {
      ...
      txs = querySnapshotNew.docs.map(doc => {
        const data = doc.data();
        return {
          id: doc.id,
          ...data,
